...
const { configFromPath } = require('./util.js');
...
async function produceExample() {
  ...
  let topic = "otot-d-topic";

  let students = [ "David", "Rahul", "Sarah", "Alvin", "Clarissa", "Joe" ];
  let pattern = [ "Factory Method", "Builder", "Sigleton", "Command", "Adpater" ];
  ...
  for (let idx = 0; idx < numEvents; ++idx) {
    ...
    const key = students[Math.floor(Math.random() * students.length)];
    const value = Buffer.from(pattern[Math.floor(Math.random() * pattern.length)]);
